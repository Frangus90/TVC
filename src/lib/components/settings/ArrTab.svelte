<script lang="ts">
  ...
  const servers = $derived(getServers());
  const editingServer = $derived(getEditingServer());
  const testingConnection = $derived(isTestingConnection());
...
<div class="flex flex-col h-full">
  <!-- Sub-tabs -->
  <div class="flex border-b border-border -mx-6 -mt-6 px-6 mb-4">
    <button
      type="button"
  ...
  <!-- Footer messages -->
  {#if error}
    <div class="p-3 mt-4 rounded-lg bg-red-500/10 text-red-400 text-sm text-center">
      {error}
    </div>
  {:else if successMessage}
    <div class="p-3 mt-4 rounded-lg bg-available/10 text-available text-sm text-center">
      {successMessage}
    </div>
  {/if}
</div>
